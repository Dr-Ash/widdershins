...
const runn  = import("@mermaid-js/mermaid-cli");
const fs = require('fs');
...
const diagrams = 'stds/';
const stds = fs.readdirSync(diagrams);
...
function insertMermaid(data){
    const mermPath = `${data.methodUpper}-${data.url.replaceAll('/', '-')}.mmd`;

    if(stds.includes(mermPath)){
        return true;
        let diag  = fs.readFileSync(diagram,  'utf8')
         return `\`\`\`mermaid\n${diag} \n\`\`\``;
    }
    return false;
}
